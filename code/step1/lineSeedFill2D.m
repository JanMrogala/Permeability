...
function ret = lineSeedFill2D(xi, yi, fillColor, boundaryColor, image)
  ...
  assert (!(isequal(reshape(image(yi,xi,:), [], 1), boundaryColor) || isequal(reshape(image(yi,xi,:), [], 1), fillColor)),
  ...
  stack = {};
	...
	while (size(stack)(1,2) != 0)
		...
		while ((x-1) > 0 &&
         !isequal(reshape(image(y,x-1,:), [], 1), boundaryColor) &&
         !isequal(reshape(image(y,x-1,:), [], 1), fillColor))
		...
		if ((y - 1) > 0 &&
        !isequal(reshape(image(y-1,x,:), [], 1), boundaryColor) &&
        !isequal(reshape(image(y-1,x,:), [], 1), fillColor))

			# push pixel into stack if there is no boundary pixel on the left, or x is out of bounds
			if ((x - 1) > 0 &&
          !isequal(reshape(image(y-1,x-1,:), [], 1), boundaryColor) ||
          (x - 1) <= 0)
		...
		while (x <= size(image)(1,2) &&
          !isequal(reshape(image(y,x,:), [], 1), boundaryColor) &&
          !isequal(reshape(image(y,x,:), [], 1), fillColor))
